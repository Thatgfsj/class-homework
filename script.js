...
const homeworkData = {
  homeworks: [
    ...
    },
    {
      ...
      subject: "文学概论",
      title: "文学概论期中作业",
      description: "用结构主义方法分析一部作品，稿纸手写，扫描成PDF提交。\n\n截止时间：5月7日\n\n---\n\n📌 AI 参考提示词（方便用AI辅助完成作业）：\n\n# 任务\n请用"1+1+1+… = 1"的结构主义方法分析（（（这里是你的作品名）））。\n\n# 核心理念\n把作品拆成多个彼此对立或差异显著的单元（每个单元是一个“1”）。这些单元可以来自：人物情绪变化、故事发展阶段、同一角色的不同侧面、不同价值观的对撞等——具体从哪个角度拆，由你自己决定。\n\n拆出来的“1”的数量至少为4个。每个“1”之间必须有清晰的对立或差异关系。最后要论证：所有这些看似分散的碎片，如何共同整合成一个统一的主题（那个“=1”）。\n\n# 输出格式（严格遵守）\n\n1. **第一段**：说明你选择了哪部作品，以及你决定从什么角度拆解（例如：“我选择从主角的心理状态变化拆出五个‘1’”或“我从两种对立价值观的反复拉锯中拆出四个回合”）。\n\n2. **第二段**：用一个表格展示你拆出的所有“1”。\n - 表格的第一列是“单元序号 + 单元名称”（例如“1. 幻想期”）。\n - 其余列是你自行选择的**3-5个对比维度**。注意：不能使用“身份、抱负、处世、勇气、人生”这五个词。\n 推荐可选的维度：内在驱动力、外在行为风格、对时间的感受、对金钱/物质的态度、对他人目光的依赖程度、语言虚实比例、身体劳动程度、幻想与现实的交集方式、情绪主色调等。\n - 每个单元格填写该单元在该维度上的简要描述。\n\n3. **第三段及之后**：用自然段落逐一描述每个“1”之间的对立关系，不需要再重复表格内容，而是解释为什么它们是“1”以及它们如何相互否定或递进。\n\n4. **最后一段**：总结论证 **1+1+1+… = 1**，即所有碎片如何构成一个统一主题。字数不限，但需完整。",
